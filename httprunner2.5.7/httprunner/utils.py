...
def ensure_mapping_format(variables):
    ...
    Args:
        variables (list/dict): original variables原始变量
...
def get_testcase_io(testcase):
    ...
    Args:
        ...
        testcase (unittest.suite.TestSuite):对应一个YAML/JSON文件，已经设置了两个属性:
            config: parsed config block解析配置块
            runner: initialized runner.Runner() with config用配置初始化runner.Runner()
    Returns:
...
def create_scaffold(project_name):
    """ create scaffold with specified project name.
        创建脚手架
    """
    if os.path.isdir(project_name): # 文件夹已存在
...
def gen_cartesian_product(*args):
    ...
    Returns:
        list: cartesian product in list表中的笛卡尔积
...
def prettify_json_file(file_list):
    ...
    for json_file in set(file_list):
        if not json_file.endswith(".json"):  # 没有json格式文件
...
def prepare_dump_json_file_abs_path(project_mapping, tag_name):
    ...
    """
    pwd_dir_path = project_mapping.get("PWD") or os.getcwd() # os.getcwd() 方返回当前工作目录
    ...
    # 相对地址，如：testcases\检查经营地址是否重复.yml
