...
def parse_string_value(str_value):
    ...
    """
    try:
        # eval既可以做string与list,tuple,dict之间的类型转换
        # ast.literal_eval 也可以，而且更安全
...
def is_var_or_func_exist(content):
    ...
    try:
        match_start_position = content.index("$", 0)  # 第一个$的下标
    except ValueError:
        return False
...
def regex_findall_variables(content):
    """ extract all variable names from content, which is in format $variable
        从content中提取所有变量名，其格式为$variable，${variable}
...
def parse_parameters(parameters, variables_mapping=None, functions_mapping=None):
    ...
    parameters = utils.ensure_mapping_format(parameters)  # 把[{}{}]解析成{}
...
class LazyString(object):
    """ evaluate string lazily.
        ...
        var = LazyString(ABC$var_1{)
        var._string = "ABC{}{{"
        var._args = ['var_1']
        var.to_value(variables_mapping) = "ABCabc{"
        例02
        functions_mapping = {
            "func1": lambda x,y: str(x) + str(y)
        }
        var = parser.LazyString("${func1($var_1, $var_3)}", functions_mapping, check_variables_set)
        var._args[0] = LazyFunction(<lambda>(LazyString($var_1), LazyString($var_3)))
    """
    ...
    def __parse(self, raw_string):
        ...
        while match_start_position < len(raw_string): # 一定为True
            ...
            # 匹配到$$,则self._string=ABC$,match_start_position+2
            if dollar_match:
                match_start_position = dollar_match.end() 
                self._string += "$"
                continue
            ...
            if func_match: # 匹配到引用函数
                ...
                self._args.append(lazy_func) # 函数执行结果？
                match_start_position = func_match.end() # 匹配下标后移
                self._string += "{}" # 函数以{}代替
                continue
...
def prepare_lazy_data(content, functions_mapping=None, check_variables_set=None, cached=False):
    ...
    配合extract_variables(content)使用
    例子：
    prepare_lazy_data("123$a", {}, {"a"}) 返回 LazyString(123$a)
    prepare_lazy_data("$a$b", {}, {"a", "b"}) 返回 LazyString($a$b)
    prepare_lazy_data(["$a$b", "$c", "d"], {}, {"a", "b", "c", "d"}) 返回 [LazyString($a$b), LazyString($c), 'd']
    parser.prepare_lazy_data(
            {"a": 1, "b": {"c": "$d", "e": 3}},
            {},
            {"d"}
        ) 返回 {'a': 1, 'b': {'c': LazyString($d), 'e': 3}}
...
def parse_lazy_data(content, variables_mapping=None):
    """ parse lazy data with evaluated variables mapping.
        ...
        {
            'request': {
                'url': LazyString(/api/users/$uid), 
                'method': LazyString($method),
                'headers': {'token': LazyString($token)},
                'data': {
                    'null': None, 
                    'true': True, 
                    'false': False, 
                    'empty_str': '', 
                    'value': LazyString(abc${add_one(3)}def)
                    }
                }
        }
        返回：
...
def extract_variables(content):
    ...
    例如:
      content: {'a': 1, 'b': {'c': LazyString($d), 'e': 3}} 返回 {'d'}
               {'a': [LazyString($b)], 'b': {'c': LazyString($d), 'e': 3}} 返回 {'b', 'd'}
...
def parse_variables_mapping(variables_mapping):
    ...
    while len(parsed_variables_mapping) != len(variables_mapping):
        for var_name in variables_mapping:
            ...
            # check if reference variable itself 检查是否引用变量本身
    ...
    [
        {
            'config': {
                'name': '',
                'base_url': LazyString($host1),
                'variables': {
                    'host1': 'https://github.com',
                    'var_a': LazyString(${gen_random_string(5)}),
                    'var_b': LazyString($var_a)
                }
            },
            'teststeps': [
                {
                    'name': 'testcase1',
                    'base_url': LazyString($host2),
                    'request': {
                        'url': LazyString(/api1/?num1=$num1&num2=$num2&num3=$num3),
                        'method': 'GET',
                        'verify': True
                    },
                    'variables': {
                        'host2': 'https://httprunner.org',
                        'num3': LazyString(${sum_two($num2, 4)}),
                        'num2': LazyString(${sum_two($num1, 3)}),
                        'num1': LazyString(${sum_two(1, 2)}),
                        'str1': LazyString(${gen_random_string(5)}),
                        'str2': LazyString($str1),
                        'host1': 'https://github.com',
                        'var_a': LazyString(${gen_random_string(5)}),
                        'var_b': LazyString($var_a)
                    },
