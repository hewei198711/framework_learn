# -*- coding: utf-8 -*-
"""
    generate_case_csv_file_jira_chandao.py
...
class ZentaoSession(requests.Session):
    url_root = 'http://172.16.1.55:7001/zentao/my/'
...
def parser_tree_to_list(root_value, tree, current_layer, pre_topics, case_list, version):
    if 'topics' not in tree:
        pre_topics.pop()
        found_max_layer = current_layer  # 最后一层=当前层
        return tree['title'], found_max_layer
    else:
        found_max_layer = 0
        tail_topic_list = []  # 尾巴主题列表
        for node in tree['topics']:  # 遍历列表中的字典
        ...
        layer_to_final = found_max_layer - current_layer  # 最终层=最后一层-当前层
        if layer_to_final == 2:
            # 生成步骤和预期
            ...
            # 生成用例
            case_list.append([
                case_module, version.upper(), pre_topics[-3].upper(), keyword.upper(), pre_topics[-1],
                ':'.join(pre_topics[-5:-3]),
                '\n'.join(step_list), '\n'.join(expect_list),
                "功能测试", "系统功能验证测试"
            ])
...
def tree_to_case_list(tree, root_value):
    ...
    for topic in tree['topic']['topics']:
        parser_tree_to_list(
            ...
            pre_topics=[topic['title']], case_list=_case_list, version="V2.0"
        )
    return _case_list
...
def generate_case(xmind_file, save_path):
    xmind_tabs = xmind_to_dict(xmind_file)
    xmind_tree = xmind_tabs[0]
    root_topic_value = xmind_tree['topic']['title']  # 根主题
    # 按用例组织excel的行
    case_list = tree_to_case_list(xmind_tree, root_topic_value)
    # 生成csv格式的测试用例
    csv_case_list = [[
        "测试用例集", "影响版本", "优先级", "需求", "描述",
        "标题", "步骤", "期望结果", "测试类型", "测试阶段"
    ]]
...
if __name__ == '__main__':
    ...
    if xmind_file[-6:] == '.xmind' and os.path.exists(xmind_file):  # 判断文件存在，格式是Xmind
        generate_case(xmind_file, save_path=target_path)   # 生成用例
    elif xmind_file[-4:] == '.csv' and os.path.exists(xmind_file):  # 判断文件存在，格式是csv
    ...
    if len(sys.argv) > 3:
        if len(sys.argv) < 6:
            ...
            ligeit_product_name = sys.argv[3]
            zentao_account = sys.argv[4]
            zentao_password = sys.argv[5]
            # 创建模块
