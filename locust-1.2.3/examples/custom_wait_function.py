...
def strictExp(min_wait, max_wait, mu=1):
    """
    Returns an exponentially distributed time strictly between two bounds.返回严格在两个界限之间的指数分布时间。
    """
    while True:
...
class StrictWebsiteUser(HttpUser):
    """
    User class that makes exponential requests but strictly between two bounds.发出指数请求但严格在两个界限之间的用户类
    """
